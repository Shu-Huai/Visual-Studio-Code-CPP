...
#include <iostream>
ProcessScheduling::ProcessScheduling(int number) : m_number(number), m_head(0), m_tail(0)
{
...
void ProcessScheduling::Display()
{
    cout << "=====================================================" << endl
         << setw(16) << "标识符：";
    for (int i = 1; i <= m_number; i++)
    ...
    if (m_running != -1)
    {
        cout << "正在运行的进程：" << m_running << endl;
    }
    else
    {
        cout << "无当前正在运行的进程。" << endl;
    }
    cout << "等待队列：";
...
void ProcessScheduling::PriorityScheduling()
{
