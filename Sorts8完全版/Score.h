...
struct Score
{
	...
	bool operator> (const Score &s) const { return Total >  s.Total; }	// 从这6个函数体可见，实际上是比较Total
	bool operator>=(const Score &s) const { return Total >= s.Total; }
	bool operator==(const Score &s) const { return Total == s.Total; }
	bool operator!=(const Score &s) const { return Total != s.Total; }
};

void InitScore(Score *data, int n);										// 设置原始数据
void ShowScore(const Score *data, int size, int last);					// 输出数组的最后last个元素
void TestScore();														// 测试函数
