...
protected:
    ...
    void CreateByPreOrder(AVLTreeNode<ElemType> *&root, ElemType *elems, int &index, int length);
...
public:
    ...
    void CreateByPreOrder(ElemType *elems, int length);
...
template <class ElemType>
void AVLTree<ElemType>::CreateByPreOrder(AVLTreeNode<ElemType> *&root, ElemType *elems, int &index, int length)
{
    if (elems[index] != referenceValue_ && index < length)
    {
        root = new AVLTreeNode<ElemType>(elems[index]);
        CreateByPreOrder(root->leftChild_, elems, ++index, length);
        CreateByPreOrder(root->rightChild_, elems, ++index, length);
    }
    else
    {
        root = NULL;
...
template <class ElemType>
void AVLTree<ElemType>::CreateByPreOrder(ElemType *elems, int length)
{
    int index = 0;
    CreateByPreOrder(root_, elems, index, length);
    GetLeftNodeNumber();
}
template <class ElemType>
